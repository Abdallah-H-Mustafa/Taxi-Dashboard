...
export function IconVisibilitySettings() {
  ...
  const handleToggle = (id: string, enabled: boolean) => {
    ...
    const updatedSettings = iconSettings.map(icon => 
      icon.id === id ? { ...icon, enabled } : icon
    );
    
    setIconSettings(updatedSettings);
    localStorage.setItem('dispatch_icon_settings', JSON.stringify(updatedSettings));
    toast.success("Icon settings updated");
  };

  const handleReset = () => {
    const defaultSettings = iconSettings.map(icon => ({ ...icon, enabled: true }));
    setIconSettings(defaultSettings);
