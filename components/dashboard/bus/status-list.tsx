...
import { BusTrip } from "@/lib/stores/bus-trip-store";
...
export function BusStatusList({ trips }: BusStatusListProps) {
          ...
          <div className="space-y-2">
            {paginatedTrips.map((trip, index) => (
              <div key={trip.id} className="grid grid-cols-12 gap-2 text-xs hover:bg-muted/50 p-1 rounded">
                ...
                <div className="col-span-3 font-mono">{trip.vehicleId || "—"}</div>
                <div className="col-span-3">{trip.status}</div>
                <div className="col-span-5 truncate">{trip.routeId}</div>
              </div>
            ))}
          </div>
        </div>
